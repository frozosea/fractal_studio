...
export async function PublicFooter() {
  ...
  return (
    <footer
      className="border-t border-instrument-rule bg-instrument"
    ...
    >
      <div className="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-8 sm:px-8 sm:py-10 md:flex-row md:items-center md:justify-between">
        ...
        <nav className="flex flex-wrap items-center gap-x-5 gap-y-2" aria-label={t("nav.label")}>
          <Link href="/" className="text-xs text-ink/60 transition-colors hover:text-ink/75">
            {t("nav.home")}
          </Link>
          <Link href="/tutorial" className="text-xs text-ink/60 transition-colors hover:text-ink/75">
            {t("nav.tutorial")}
          </Link>
          <Link href="/help" className="text-xs text-ink/60 transition-colors hover:text-ink/75">
            {t("nav.help")}
          </Link>
        ...
        <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-ink/25">
          {t("footer.licence")}
        </span>
      </div>
    </footer>
