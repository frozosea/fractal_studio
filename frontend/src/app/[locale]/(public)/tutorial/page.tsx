        ...
        <ol className="mt-3 space-y-1.5">
          {STEPS.map((step, index) => (
            <li key={step}>
              <a
                href={`#step-${index + 1}`}
                className="group flex items-baseline gap-3 text-sm text-white/50 transition-colors hover:text-white/85"
              >
      ...
      <div className="mt-14 space-y-14">
        {STEPS.map((step, index) => (
          <section key={step} id={`step-${index + 1}`} className="scroll-mt-20">
            ...
            <h2 className="mt-3 text-xl font-light tracking-tight text-white/90 sm:text-2xl">
              {t(`steps.${step}.name`)}
            </h2>
            ...
            {step === "navigate" && (
              <div className="mt-6 grid gap-px bg-white/10 sm:grid-cols-2">
                {GESTURES.map((gesture) => (
                  <div key={gesture} className="min-w-0 bg-[#0b0d10] p-4">
                    <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-amber-200/70">
                      {t(`gestures.${gesture}.input`)}
                    </p>
                    <p className="mt-2 text-sm text-white/70">{t(`gestures.${gesture}.effect`)}</p>
                  </div>
                ))}
              </div>
            )}
