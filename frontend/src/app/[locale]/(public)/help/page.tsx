...
export default async function HelpPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-14 sm:px-8 sm:py-20">
      ...
      <p className="mt-5 max-w-2xl text-sm leading-relaxed text-white/55">{t("lede")}</p>
      ...
      <div className="mt-12 space-y-12">
        {GROUPS.map((group, index) => (
          <section key={group.id} id={`group-${group.id}`} className="scroll-mt-20">
            ...
            <div className="mt-4 space-y-px bg-white/10">
              {group.questions.map((question) => (
                <details key={question} className="group min-w-0 bg-[#0b0d10]">
                  <summary className="flex cursor-pointer list-none items-start gap-3 p-4 text-sm text-white/80 transition-colors hover:text-white coarse:py-5">
                    <ChevronRight
                  ...
                  <div className="px-4 pb-4 pl-11 text-sm leading-relaxed text-white/55">
      ...
      <div className="mt-8">
        <p className="text-sm text-white/55">{t("more.body")}</p>
